...
function AdminHeader({ onNavigate }: AdminHeaderProps) {
  return (
    ...
    <Card className="border rounded-2xl overflow-hidden bg-card">
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-[10px] text-muted-foreground flex items-start gap-1">
          ...
          <span>
            Store configuration is loaded from `/api/pos-config` dynamic
            registry endpoint.
          </span>
        </p>
      </CardContent>
...
interface RegistryMaintenanceCardProps {
  onMaintenanceAction: (action: string) => void;
}

function RegistryMaintenanceCard({
  onMaintenanceAction,
}: RegistryMaintenanceCardProps) {
  return (
    <Card className="border rounded-2xl overflow-hidden bg-card">
      <CardHeader>
        <CardTitle className="text-sm font-bold flex items-center gap-2">
      ...
      <CardContent className="space-y-2">
        <Button
          ...
          onClick={() => onMaintenanceAction("Re-index Menu Catalog")}
        >
        ...
        <Button
          ...
          onClick={() => onMaintenanceAction("Purge Active Workspace Branches")}
        >
        ...
        <Button
          ...
          className="w-full justify-start text-xs h-9 rounded-lg"
          onClick={() =>
            onMaintenanceAction("Rebuild Jurisdictional Surcharges")
          }
        >
...
interface PreferencesCardProps {
  localPrefs: any;
  setLocalPrefs: React.Dispatch<React.SetStateAction<any>>;
  onOpenCatalogDetail: () => void;
  onOpenSplitWarnPad: () => void;
}
  ...
  return (
    <Card className="border rounded-2xl overflow-hidden bg-card md:col-span-2">
      <CardHeader className="border-b bg-muted/20">
        ...
        <CardDescription className="text-xs">
          Configure interface presets, behavior flags, and custom color mappings
          applied globally.
        </CardDescription>
      </CardHeader>
      <CardContent className="p-6 space-y-6 divide-y divide-border/60">
        ...
        <div className="space-y-4 pt-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground">
                  Sets default detail rendering mode
                </p>
              </div>
              <Select
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="ledger-collapsed"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Ledger Panel Collapsed
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Collapse checkout log panel by default
                </p>
              </div>
              <Checkbox
                id="ledger-collapsed"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="group-notes-collapsed"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Group Notes Collapsed
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Collapse group annotation sidebar
                </p>
              </div>
              <Checkbox
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground">
                  Render tighter element paddings
                </p>
              </div>
              <Checkbox
                id="compact-mode"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="use-comma-decimal"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Use Comma for Decimal
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Format numbers using commas (e.g., 12,34)
                </p>
              </div>
              <Checkbox
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="catalog-detail-level"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Catalog Layout Configuration
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Customize columns, rows, navigation details
                </p>
              </div>
              <Button
        ...
        <div className="space-y-4 pt-6">
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">
            Order & POS Logic Settings
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="inline-mod-price-display"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Inline Modifier Price Display
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Toggle delta vs net price visualization
                </p>
              </div>
              <div className="inline-flex items-stretch rounded-full border bg-background p-1 shadow-sm">
                ...
                <button
                  ...
                  onClick={() =>
                    setLocalPrefs((prev: any) => ({
                      ...prev,
                      inlineModifierPriceDisplayDelta: false,
                    }))
                  }
                  className={`min-w-16 rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-wide transition-colors ${!localPrefs.inlineModifierPriceDisplayDelta ? "bg-primary text-primary-foreground shadow-xs" : "text-muted-foreground hover:text-foreground"}`}
                >
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="default-multi-select"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Default to Multi-Select
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Select multiple tickets by default
                </p>
              </div>
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setLocalPrefs((prev: any) => ({
                    ...prev,
                    defaultMultiSelectMode: !!checked,
                  }))
                }
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="auto-select-last-clicked"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Auto-Select Last Clicked Item
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Focus automatically on newly added items
                </p>
              </div>
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setLocalPrefs((prev: any) => ({
                    ...prev,
                    autoSelectLastClickedItem: !!checked,
                  }))
                }
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label
                  htmlFor="split-warn-threshold"
                  className="text-xs font-semibold uppercase text-muted-foreground cursor-pointer"
                >
                  Split Line Warn Threshold
                </label>
                <p className="text-[10px] text-muted-foreground">
                  Line quantity triggering split warn dialogs
                </p>
              </div>
              <button
                id="split-warn-threshold"
                onClick={onOpenSplitWarnPad}
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground">
                  Colors for ticket nested modifiers layout hierarchy depth
                  levels
                </p>
              </div>
              <Button
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {localPrefs.globalDepthColors?.map(
                (color: string, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center gap-1 bg-muted/30 p-1 rounded-md border shadow-sm"
                  >
                    <Input
                      type="color"
                      value={color}
                      onChange={(e) => {
                        const newColors = [
                          ...(localPrefs.globalDepthColors || []),
                        ];
                        newColors[idx] = e.target.value;
                        setLocalPrefs((prev: any) => ({
                          ...prev,
                          globalDepthColors: newColors,
                        }));
                      }}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setLocalPrefs((prev: any) => ({
                          ...prev,
                          globalDepthColors:
                            newColors.length > 0 ? newColors : ["#94a3b8"],
                        }));
                      }}
                    >
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border/40">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground">
                  Color designations for guest/seat tracking tags
                </p>
              </div>
              <Button
                variant="ghost"
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {localPrefs.globalGuestPalette?.map(
                (color: string, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center gap-1 bg-muted/30 p-1 rounded-md border shadow-sm"
                  >
                    <Input
                      ...
                      onChange={(e) => {
                        const newColors = [
                          ...(localPrefs.globalGuestPalette || []),
                        ];
                        newColors[idx] = e.target.value;
                        setLocalPrefs((prev: any) => ({
                          ...prev,
                          globalGuestPalette: newColors,
                        }));
                      }}
                      className="w-8 h-8 p-0.5 cursor-pointer border-none bg-transparent"
                    />
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setLocalPrefs((prev: any) => ({
                          ...prev,
                          globalGuestPalette:
                            newColors.length > 0 ? newColors : ["#10b981"],
                        }));
                      }}
                    >
                      <X className="w-3.5 h-3.5" />
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border/40">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground">
                  VCS branching visualization branch line tags
                </p>
              </div>
              <Button
                variant="ghost"
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {localPrefs.globalBranchColors?.map(
                (color: string, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center gap-1 bg-muted/30 p-1 rounded-md border shadow-sm"
                  >
                    <Input
                      ...
                      onChange={(e) => {
                        const newColors = [
                          ...(localPrefs.globalBranchColors || []),
                        ];
                        newColors[idx] = e.target.value;
                        setLocalPrefs((prev: any) => ({
                          ...prev,
                          globalBranchColors: newColors,
                        }));
                      }}
                      className="w-8 h-8 p-0.5 cursor-pointer border-none bg-transparent"
                    />
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setLocalPrefs((prev: any) => ({
                          ...prev,
                          globalBranchColors:
                            newColors.length > 0 ? newColors : ["#3b82f6"],
                        }));
                      }}
                    >
                      <X className="w-3.5 h-3.5" />
...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background select-none">
      <AdminHeader onNavigate={(path) => router.push(path)} />

      {/* Main Grid Content */}
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 overflow-y-auto space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <GeneralSettingsCard />
          <RegistryMaintenanceCard
            onMaintenanceAction={handleMaintenanceAction}
          />
          <PreferencesCard
            localPrefs={localPrefs}
            setLocalPrefs={setLocalPrefs}
            onOpenCatalogDetail={() => setCatalogDetailOpen(true)}
            onOpenSplitWarnPad={() => setSplitWarnPadOpen(true)}
          />
        </div>
      </main>

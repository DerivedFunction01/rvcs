...
export function GuestPickerDialog({ open, onOpenChange, selectedPerson, onSelectPerson, onEditGuest, onOpenAddGuest }: any) {
              ...
              {filtered.map((guest: any, idx: number) => (
                <div key={guest.id} onClick={() => { onSelectPerson(guest.id); onOpenChange(false); }} className={`relative group flex flex-col items-start gap-1 rounded-lg border p-3 text-left transition-all hover:border-primary/50 hover:bg-accent/40 cursor-pointer ${selectedPerson === guest.id ? "border-primary bg-primary/5" : "bg-card"}`}>
                  <div className="flex items-center justify-between w-full">
                    <span className={`w-2.5 h-2.5 rounded-full ${GUEST_PALETTE[idx % GUEST_PALETTE.length]}`} />
                  ...
                  <span className="w-full truncate text-sm font-semibold">{guest.name}</span>
                </div>
              ))}
            </div>
          )}
...
export function EditGuestDialog({ open, onOpenChange, guestToEdit }: any) {
  ...
  useEffect(() => { if (open && guestToEdit) { setAlias(guestToEdit.name || ""); } }, [open, guestToEdit]);
  const handleSave = () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-2"><div className="space-y-2"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Guest Name</label><Input autoFocus value={alias} onChange={(e) => setAlias(e.target.value)} placeholder={`Guest name`} onKeyDown={(e) => { if (e.key === "Enter") { handleSave(); } }} /></div></div>
        <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button onClick={handleSave}>Save</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
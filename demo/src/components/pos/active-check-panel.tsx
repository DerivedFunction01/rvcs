        ...
        <div ref={bulkActionsBarRef} className="mx-4 my-2 p-3 bg-card/85 backdrop-blur-md border rounded-xl shadow-lg flex flex-col gap-3 animate-in slide-in-from-bottom-2 duration-200 shrink-0">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <div className="flex items-center gap-1 bg-muted/30 border p-1 rounded-lg">
              ...
              <Button variant="ghost" size="sm" className="h-7 text-[11px] px-2.5 font-medium bg-background border shadow-sm hover:bg-accent" onClick={() => { setAssignmentAllocationItems(Array.from(selectedLineIds).map(id => projectedState.items[id as string]).filter(Boolean)); setAssignmentAllocationContext(AllocationContext.Group); setAssignmentAllocationOpen(true); }}>Guest</Button>
              <Button variant="ghost" size="sm" className="h-7 text-[11px] px-2.5 font-medium bg-background border shadow-sm hover:bg-accent" onClick={() => { setPaymentAllocationItems(Array.from(selectedLineIds).map(id => projectedState.items[id as string]).filter(Boolean)); setPaymentAllocationContext(AllocationContext.Group); setPaymentAllocationOpen(true); }}>Payment</Button>
              <Button variant="ghost" size="sm" className="h-7 text-[11px] px-2.5 font-medium bg-background border shadow-sm hover:bg-accent" onClick={() => { setFulfillmentAllocationItems(Array.from(selectedLineIds).map(id => projectedState.items[id as string]).filter(Boolean)); setFulfillmentAllocationContext(AllocationContext.Group); setFulfillmentAllocationOpen(true); }}>Fulfillment</Button>

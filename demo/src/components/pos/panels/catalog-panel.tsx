...
export function CatalogPanel({
  ...
  const gridContainerStyle = {
    ...
    gridTemplateRows: navigationMode === CatalogNavigationMode.Page ? `repeat(${gridRows}, minmax(0, 1fr))` : undefined,
  ...
  return (
    <aside className={`${widthClass} border-r bg-card flex flex-col shrink-0 h-full overflow-hidden`}>
      {/* Top Filter and Search Controls */}
      {SearchControls(setCatalogLayoutOpen, detailDisplay, navigationMode, categoryMode, gridRows, gridCols, catalogFilter, setCatalogFilter, requireTags, avoidTags, setRequireTags, setAvoidTags, availableTags, iconConfigs, flatFilteredItems)}
      ...
      <div className="flex-1 min-h-0 flex flex-row p-2 gap-3 dynamic-content-area">
        ...
        {isCategoryMode && (
          getCategoryColumn(currentCategoryPage, categoryPageCount, visibleCategories, activeCategory, setActiveCategory, setCategoryPage)
        )}
        ...
        <div className="flex-1 flex flex-col min-h-0 w-full min-w-0">
          <ScrollArea className="flex-1 min-h-0 w-full">
            <div className="grid gap-2 w-full pb-2" style={gridContainerStyle}>
          ...
          <div className="h-14 flex items-center justify-stretch pt-2 border-t mt-auto shrink-0 w-full min-w-0">
            {navigationMode === CatalogNavigationMode.Page ? (
              <div className="grid grid-cols-2 gap-2 w-full">
                {currentPage > 0 ? (
                  <Button variant="outline" className="h-12 w-full" onClick={() => setPage((prev) => Math.max(0, prev - 1))}>
                    <ChevronLeft className="w-4 h-4 mr-1" />
                    Prev Page
                  </Button>
                ) : (
                  <Button variant="outline" className="h-12 opacity-0 pointer-events-none w-full" tabIndex={-1} aria-hidden="true">
                    Prev Page
                  </Button>
                )}
                {currentPage + 1 < pageCount ? (
                  <Button variant="outline" className="h-12 w-full" onClick={() => setPage((prev) => Math.min(pageCount - 1, prev + 1))}>
                    Next Page
...
function getCategoryColumn(currentCategoryPage: number, categoryPageCount: number, visibleCategories: { category: string; items: CatalogItemEntry[]; }[], activeCategory: string | null, setActiveCategory: Dispatch<SetStateAction<string | null>>, setCategoryPage: Dispatch<SetStateAction<number>>): import("react").ReactNode {
  return <div className="w-36 xl:w-40 shrink-0 flex flex-col h-full border-r pr-2">
    ...
    <div className="flex-1 flex flex-col gap-2 min-h-0">
        ...
        {visibleCategories.map((section) => {
          ...
          return (
            <Button
              ...
              className="h-full min-h-10 px-3 text-sm justify-between w-full"
      ...
      <div className="grid grid-cols-2 gap-2 pt-1 shrink-0 h-11 items-center">
        {currentCategoryPage > 0 ? (
          <Button variant="ghost" className="h-10 w-full" onClick={() => setCategoryPage((prev) => Math.max(0, prev - 1))}>
            <ChevronLeft className="w-4 h-4 mr-1" />
            Prev
          </Button>
        ) : (
          <Button variant="ghost" className="h-10 opacity-0 pointer-events-none w-full" tabIndex={-1} aria-hidden="true">
            Prev
          </Button>
        )}
        {currentCategoryPage + 1 < categoryPageCount ? (
          <Button variant="ghost" className="h-10 w-full" onClick={() => setCategoryPage((prev) => Math.min(categoryPageCount - 1, prev + 1))}>
            Next
            <ChevronRight className="w-4 h-4 ml-1" />
          </Button>
        ) : (
...
function SearchControls(setCatalogLayoutOpen: Dispatch<SetStateAction<boolean>>, detailDisplay: CatalogDetailDisplayPrefs, navigationMode: CatalogNavigationMode, categoryMode: CatalogCategoryMode, gridRows: number, gridCols: number, catalogFilter: string, setCatalogFilter: Dispatch<SetStateAction<string>>, requireTags: Set<string>, avoidTags: Set<string>, setRequireTags: Dispatch<SetStateAction<Set<string>>>, setAvoidTags: Dispatch<SetStateAction<Set<string>>>, availableTags: string[], iconConfigs: Record<string, IconConfig>, flatFilteredItems: CatalogRow[]) {
  return <div className="p-3 border-b space-y-2 shrink-0">
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          ...
          className="h-28 w-full rounded-lg border bg-background p-2 text-left hover:border-primary/40 hover:bg-accent/40 transition-colors flex flex-col justify-between gap-2 overflow-hidden shrink-0"
        >
          <div className="space-y-1 min-w-0 flex-1 min-h-0 w-full">
            <div className="flex items-start justify-between gap-2 w-full">
              <span className="text-sm font-medium leading-tight flex-1">{item.name}</span>
          ...
          <div className="flex items-center justify-between gap-2 text-[10px] text-muted-foreground w-full mt-auto">
            <span className="uppercase tracking-wider truncate flex-1">{category}</span>
            <span className="text-primary font-medium shrink-0">Add</span>
          </div>
        </button>
      </TooltipTrigger>

...
interface CustomerSearchDialogProps {
  ...
  onSelectCustomer: (data: { name: string; phone: string; address: string }) => void;
}
...
export function CustomerSearchDialog({
  ...
  const handleSelect = (customer: CustomerProfile) => {
    ...
    const phoneContact = customer.contacts.find((c) => c.channel === "phone" && c.isPrimary) ||
                          customer.contacts.find((c) => c.channel === "phone") ||
                          customer.contacts[0];
    const emailContact = customer.contacts.find((c) => c.channel === "email");
    const defaultLocation = customer.deliveryLocations.find((l) => l.isDefault) ||
  ...
  const getTierBadge = (tier: string) => {
    switch (tier.toLowerCase()) {
      case "gold":
        return <Badge className="bg-amber-500 hover:bg-amber-600 text-white border-none text-[10px]">Gold</Badge>;
      case "silver":
        return <Badge className="bg-slate-400 hover:bg-slate-500 text-white border-none text-[10px]">Silver</Badge>;
      case "bronze":
        return <Badge className="bg-orange-600 hover:bg-orange-700 text-white border-none text-[10px]">Bronze</Badge>;
      default:
        ...
        <ScrollArea className="flex-1 min-h-0 max-h-[45vh] -mx-6 px-6">
          <div className="space-y-2">
            {customers.length === 0 ? (
              ...
              customers.map((c) => {
                const displayName = c.names[0]?.displayName || "Unknown Customer";
                const phoneContact = c.contacts.find((con) => con.channel === "phone")?.value;
                const emailContact = c.contacts.find((con) => con.channel === "email")?.value;
                const defaultAddress = c.deliveryLocations.find((l) => l.isDefault)?.formattedAddress || c.deliveryLocations[0]?.formattedAddress;
                      ...
                      <div className="flex flex-col gap-0.5 text-xs text-muted-foreground pl-5">
                        {phoneContact && (
                          <span className="flex items-center gap-1.5">
                            <Phone className="w-3 h-3 shrink-0" /> {phoneContact}
                          </span>
                        )}
                        {emailContact && (
                          <span className="flex items-center gap-1.5">
                        ...
                        {defaultAddress && (
                          <span className="flex items-center gap-1.5 mt-0.5 text-[11px] font-medium text-foreground/80">
                            <MapPin className="w-3 h-3 shrink-0 text-primary" /> {defaultAddress}
                          </span>
                        )}
                      </div>
                    </div>
